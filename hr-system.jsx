...
export default function HRSystem() {
  ...
  useEffect(()=>{
    (async()=>{
      try { const r=await window.storage.get("company-settings"); if(r){const p=JSON.parse(r.value);setCompany(p);setLocalCompany(p);} } catch{}
      setLoading(false);
    })();
  },[]);
  ...
  const saveCompany = async (d) => {
    setCompany(d); try{await window.storage.set("company-settings",JSON.stringify(d));}catch{}
